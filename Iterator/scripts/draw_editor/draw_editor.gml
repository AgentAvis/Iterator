...
function draw_editor(){
	...
	if color_picker {
		...
		sprite_index = sColor_picker //set the sprite to the current menu's button image
		...
		repeat (image_number) {
			...
			var _xoff = 2+(((-spr_wid)+(_temp_wid/2)));
			var _yoff = 0;
			...
			draw_sprite_ext(sprite_index,_i,x,y,_sc,_sc,image_angle,color,image_alpha) //draw the button
			x -= _xoff;
			y -= _yoff;			
			_i ++;
		}
			...
			if _i = 0 or _i = 2 or _i = 3 { // am I a valid button
				...
				if collision_point(mouse_x,mouse_y,self,true,false) { //am I being hovered over
					...
					if _i = 0 and _click{
						...
						color_picker_target = oCan
					}
					//_yoff = 1;
				}
			}
			draw_sprite_ext(sprite_index,_i,x+_xoff,y+_yoff,_sc,_sc,image_angle,color,image_alpha) //draw the button
